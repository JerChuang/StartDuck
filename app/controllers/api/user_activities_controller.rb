class Api::UserActivitiesController < ApplicationController
  def index
    ...
    # @activities = @user_activities.select('user_activities.id AS user_activities_id, user_activities.*, activities.*').joins("INNER JOIN activities ON user_activities.activity_id = activities.id" )
    @activities = @user_activities.joins(:activity)
